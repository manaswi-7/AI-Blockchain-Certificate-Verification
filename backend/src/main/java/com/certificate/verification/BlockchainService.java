...
public class BlockchainService {
  ...
  public boolean configured() {
    return !rpcUrl.isBlank() && !privateKey.isBlank() && !contractAddress.isBlank();
  }
  ...
  private byte[] bytes32(String hex) {
    String h = hex.startsWith("0x") ? hex.substring(2) : hex;
    return Numeric.hexStringToByteArray(h);
  }

  public String anchor(String certificateIdHash, String documentHash) throws Exception {
    if (!configured()) {
      throw new IllegalStateException("Blockchain is not configured. Set BLOCKCHAIN_RPC_URL, BLOCKCHAIN_PRIVATE_KEY and BLOCKCHAIN_CONTRACT_ADDRESS.");
    }
    ...
    Function f = new Function("anchorCertificate",
        java.util.List.of(
            new Bytes32(bytes32(certificateIdHash)),
            new Bytes32(bytes32(documentHash))),
        java.util.List.of());
  ...
  public boolean verify(String certificateIdHash, String documentHash) throws Exception {
    if (!configured()) {
      throw new IllegalStateException("Blockchain is not configured.");
    }
    ...
    var response = w.ethCall(
        Transaction.createEthCallTransaction(null, contractAddress, FunctionEncoder.encode(f)),
