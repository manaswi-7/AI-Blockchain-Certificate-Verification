...
public class CertificateController {
  ...
  public Map<String,Object> issue(@RequestParam String certificateId, @RequestParam String recipientName,
      @RequestParam String course, @RequestParam String institution, @RequestParam(required=false) String issueDate,
      @RequestParam(required=false, defaultValue="VALID") String validity, @RequestPart MultipartFile file) throws Exception {
    ...
    if (!("application/pdf".equals(type)||"image/png".equals(type)||"image/jpeg".equals(type))) throw new IllegalArgumentException("Only PDF, PNG and JPEG certificates are supported");
    ...
    try { Map<?,?> result=ai.analyze(file.getBytes(),file.getOriginalFilename()==null?"certificate":file.getOriginalFilename()); db.update("INSERT INTO ai_results(certificate_id,classification,confidence,extracted_fields,issues,recommendation) VALUES(?,?,?,?,?::jsonb,?)",certificateId,String.valueOf(result.get("classification")),toNumber(result.get("confidence")),String.valueOf(result.getOrDefault("extracted_fields",Map.of())).replace('=',':'),String.valueOf(result.getOrDefault("issues",java.util.List.of())),String.valueOf(result.getOrDefault("recommendation","AI analysis completed"))); } catch(Exception ignored) { }
  ...
  public Map<String,Object> verify(@PathVariable String certificateId, @RequestParam(required=false) String hash) {
    ...
    try { chainMatch=blockchain.verify(sha256(certificateId),String.valueOf(c.get("document_hash"))); chainReason=chainMatch?"Blockchain hash matched":"Blockchain hash did not match"; } catch(Exception e) { chainReason=e.getMessage()==null?"Blockchain unavailable":e.getMessage(); }
    String result=dbMatch&&chainMatch?"GENUINE":(hash!=null&&!dbMatch?"TAMPERED":"INVALID");
    String reason=dbMatch&&chainMatch?"Database and blockchain hashes matched":(!dbMatch?"Supplied document hash does not match":"Certificate could not be verified on blockchain");
  ...
  public Map<String,Object> verifyUpload(@RequestParam String certificateId, @RequestPart MultipartFile file) throws Exception {
    if (file.isEmpty()) throw new IllegalArgumentException("Certificate file is required");
    String hash=sha256(file.getBytes());
    return verify(certificateId,hash);
  }
  ...
  private double toNumber(Object x) { try{return Double.parseDouble(String.valueOf(x));}catch(Exception e){return 0.0;} }
}
