...
export default function VerifyPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      setMessage("Verification service is unavailable. Please try again later.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="page">
      ...
      <section style={{ maxWidth: 850, margin: "50px auto", padding: "0 24px" }}>
        ...
        <p className="lead">
          No certificate ID is required. Upload the certificate image and the system will use OCR
          to find the ID, AI to assess possible tampering, and blockchain to verify its registered hash.
        </p>
        ...
        {result && (
          <div className={`result ${String(result.result || "").toLowerCase()}`} style={{ marginTop: 24 }}>
            ...
            {result.aiPrediction && (
              <p><b>AI:</b> {result.aiPrediction}{typeof result.aiConfidence === "number" ? ` (${Math.round(result.aiConfidence * 100)}%)` : ""}</p>
            )}
            {typeof result.hashMatch === "boolean" && <p><b>SHA-256:</b> {result.hashMatch ? "MATCHED ✓" : "MISMATCH ✗"}</p>}
            {typeof result.blockchainMatch === "boolean" && <p><b>Blockchain:</b> {result.blockchainMatch ? "MATCHED ✓" : "NOT MATCHED ✗"}</p>}
            <p>{result.reason}</p>
            {Array.isArray(result.aiIssues) && result.aiIssues.length > 0 && (
              <div><b>AI observations:</b><ul>{result.aiIssues.map((issue: string, i: number) => <li key={i}>{issue}</li>)}</ul></div>
            )}
          </div>
        )}
      </section>
